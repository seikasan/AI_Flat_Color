...
class ImageProcessor {
  ...
  /**
   * Sobelオペレータによるエッジ検出
   * @returns {Float32Array} エッジ強度マップ (0-1)
   */
   ...
   * メディアンフィルタによるスムージング
   */
  applyMedianFilter(imageData, strength, edgeMap, edgeProtect) {
   ...
   * K-means法による色量子化
   */
  quantizeColors(imageData, colorCount, edgeMap, edgeProtect) {
    const { width, height } = imageData;
    const data = imageData.data;
    const result = new Uint8ClampedArray(data.length);
